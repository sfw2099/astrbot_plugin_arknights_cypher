...
# PRTS Wiki 反爬：屏蔽浏览器 UA（Mozilla 开头一律 403），非浏览器 UA 可正常访问
HEADERS = {
    'User-Agent': 'AstrBotArknightsCypher/1.0 (game data sync; contact via GitHub)',
...
def get_all_operator_names():
    ...
    while True:
        ...
        try:
            ...
            print(f"[fetch] get_all_operator_names 异常: {e}")
            break
...
def get_all_skin_urls(name):
    ...
    try:
        ...
        print(f"[fetch] get_all_skin_urls({name}) 异常: {e}")
...
def fetch_operator_detail(name):
    ...
    try:
        ...
        print(f"[fetch] fetch_operator_detail({name}) 异常: {e}")
        return None
